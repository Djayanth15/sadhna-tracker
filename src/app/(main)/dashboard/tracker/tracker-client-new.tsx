...
export function TrackerClientNew({ user }: TrackerClientProps) {
  ...
  const [expandedWeeklyUser, setExpandedWeeklyUser] = useState<string | null>(null);
          ...
          {view === 'admin' && isAdmin && (
            <div className='space-y-4 sm:space-y-6'>
              ...
              {adminView === 'weekly' && (() => {
                ...
                const latestWeekStart = allUsersWeeklyScores.length > 0
                  ? allUsersWeeklyScores.reduce((latest, s) =>
                      new Date(s.weekStart) > new Date(latest) ? s.weekStart : latest,
                ...
                const submittedNames = new Set(submittedRows.map((r) => r.userName));
                ...
                const allParticipantNames = new Set(allParticipants.map((p) => p.name));
                ...
                const noHistoryParticipants = allParticipants
                  .filter((p) => !submittedNames.has(p.name) && !historicButNotThisWeek.some((r) => r.userName === p.name))
                  .map((p) => ({ userName: p.name, allWeeks: [] as WeeklySummaryWithUser[] }));

                const notSubmittedRows = [...historicButNotThisWeek, ...noHistoryParticipants]
                ...
                return (
                  <div className='space-y-4'>
                    {lastWeekRows.length === 0 && notSubmittedRows.length === 0 ? (
                      <p className='text-sm text-muted-foreground text-center py-8'>
                        No weekly scores calculated yet
                      </p>
                    ) : (
                        ...
                        {latestWeekStart && (
                          <p className='text-sm text-muted-foreground px-1'>
                            ...
                            <span className='font-medium text-foreground'>
                              {format(parseLocalDate(latestWeekStart), 'MMM d')} –{' '}
                              {format(
                        ...
                        <Card>
                          <CardContent className='p-0 overflow-x-auto'>
                            <Table>
                              ...
                              <TableBody>
                                {lastWeekRows.map(({ userName, lastWeek, allWeeks }, idx) => (
                                  <>
                                    <TableRow
                                      key={userName}
                                    ...
                                    {/* Expanded history */}
                                    {expandedWeeklyUser === userName && (
                                      <TableRow key={`${userName}-expanded`}>
                                              ...
                                              {allWeeks.map((week) => (
                                                ...
                                                >
                                                  ...
                                                  <span className='text-green-600 dark:text-green-400 font-semibold'>
                                                    Overall: {week.overallAverage.toFixed(1)}%
                                                  </span>
                                ...
                                {notSubmittedRows.map(({ userName, allWeeks }) => (
                                  <>
                                    <TableRow
                                      key={`ns-${userName}`}
                                      ...
                                      onClick={() =>
                                        allWeeks.length > 0
                                          ? setExpandedWeeklyUser(
                                              expandedWeeklyUser === `ns-${userName}` ? null : `ns-${userName}`
                                            )
                                          : undefined
                                      }
                                    >
                                      ...
                                      <TableCell>
                                        {allWeeks.length > 0 && (
                                          expandedWeeklyUser === `ns-${userName}`
                                            ? <ChevronUp className='h-4 w-4 text-muted-foreground' />
                                            : <ChevronDown className='h-4 w-4 text-muted-foreground' />
                                        )}
                                      </TableCell>
                                    ...
                                    {expandedWeeklyUser === `ns-${userName}` && allWeeks.length > 0 && (
                                      <TableRow key={`ns-${userName}-expanded`}>
                                        <TableCell colSpan={7} className='p-0 bg-muted/20'>
                                          <div className='p-4 space-y-3'>
                                            <p className='text-xs font-semibold text-muted-foreground uppercase tracking-wide'>
                                            ...
                                            <div className='space-y-2'>
                                              {allWeeks.map((week) => (
                                                ...
                                                >
                                                  ...
                                                  <span className='text-purple-600 dark:text-purple-400'>Soul: {week.totalSoulScore.toFixed(1)}%</span>
                                                  <span className='text-blue-600 dark:text-blue-400'>Body: {week.totalBodyScore.toFixed(1)}%</span>
                                                  <span className='text-green-600 dark:text-green-400 font-semibold'>Overall: {week.overallAverage.toFixed(1)}%</span>
                                                  <span className='text-muted-foreground text-xs ml-auto'>{week.daysRecorded}/7 days</span>
                                                </div>
                                              ))}
                                            </div>
                                          </div>
